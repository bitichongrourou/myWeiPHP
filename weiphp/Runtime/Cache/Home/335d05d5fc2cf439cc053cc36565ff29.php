...
<?php if(!empty($normal_tips)): ?><p class="normal_tips"><b class="fa fa-info-circle"></b> <?php echo ($normal_tips); ?></p><?php endif; ?>  
      <div class="tab-content"> 
        <!-- 表单 -->
        <form id="form" action="<?php echo U('add?ctid='.$ctid.'&model='.$model['id']);?>" method="post" class="form-horizontal">
          <!-- 基础文档模型 -->
							...
							<tr>
								<td>								
									<select id="s1" name="s1" style="width:250px; height:220px;" multiple="multiple">
										<?php if(is_array($option_users)): $i = 0; $__LIST__ = $option_users;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$uinfo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($uinfo["userName"]); ?>"><?php echo ($uinfo["name"]); ?>(<?php echo ($uinfo["userName"]); ?>)</option><?php endforeach; endif; else: echo "" ;endif; ?>
									</select>
								</td>
								<td>
